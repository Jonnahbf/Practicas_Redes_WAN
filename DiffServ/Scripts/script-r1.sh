...
tc filter add dev eth0 parent ffff: protocol ip prio 1 u32 match ip src \
 192.168.1.10/32 police rate 1.2mbit burst 10k continue flowid :1

tc filter add dev eth0 parent ffff: protocol ip prio 1 u32 match ip src \
  192.168.1.10/32 police rate 600kbit burst 10k drop flowid :2

#Ancho de banda para pc2
tc filter add dev eth0 parent ffff:  protocol ip prio 1 u32 match ip src \
  192.168.1.20/32 police rate 300kbit burst 10k continue flowid :3

tc filter add dev eth0 parent ffff:  protocol ip prio 1 u32 match ip src \
  192.168.1.20/32 police rate 400kbit  burst 10k drop flowid :4
